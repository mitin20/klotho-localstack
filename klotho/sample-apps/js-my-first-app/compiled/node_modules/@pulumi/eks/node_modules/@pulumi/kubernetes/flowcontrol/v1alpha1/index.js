"use strict";
// *** WARNING: this file was generated by pulumigen. ***
// *** Do not edit by hand unless you're certain you know what you are doing! ***
var __createBinding = (this && this.__createBinding) || (Object.create ? (function(o, m, k, k2) {
    if (k2 === undefined) k2 = k;
    var desc = Object.getOwnPropertyDescriptor(m, k);
    if (!desc || ("get" in desc ? !m.__esModule : desc.writable || desc.configurable)) {
      desc = { enumerable: true, get: function() { return m[k]; } };
    }
    Object.defineProperty(o, k2, desc);
}) : (function(o, m, k, k2) {
    if (k2 === undefined) k2 = k;
    o[k2] = m[k];
}));
var __exportStar = (this && this.__exportStar) || function(m, exports) {
    for (var p in m) if (p !== "default" && !Object.prototype.hasOwnProperty.call(exports, p)) __createBinding(exports, m, p);
};
Object.defineProperty(exports, "__esModule", { value: true });
const pulumi = require("@pulumi/pulumi");
const utilities = require("../../utilities");
// Export members:
__exportStar(require("./flowSchema"), exports);
__exportStar(require("./flowSchemaList"), exports);
__exportStar(require("./flowSchemaPatch"), exports);
__exportStar(require("./priorityLevelConfiguration"), exports);
__exportStar(require("./priorityLevelConfigurationList"), exports);
__exportStar(require("./priorityLevelConfigurationPatch"), exports);
// Import resources to register:
const flowSchema_1 = require("./flowSchema");
const flowSchemaList_1 = require("./flowSchemaList");
const flowSchemaPatch_1 = require("./flowSchemaPatch");
const priorityLevelConfiguration_1 = require("./priorityLevelConfiguration");
const priorityLevelConfigurationList_1 = require("./priorityLevelConfigurationList");
const priorityLevelConfigurationPatch_1 = require("./priorityLevelConfigurationPatch");
const _module = {
    version: utilities.getVersion(),
    construct: (name, type, urn) => {
        switch (type) {
            case "kubernetes:flowcontrol.apiserver.k8s.io/v1alpha1:FlowSchema":
                return new flowSchema_1.FlowSchema(name, undefined, { urn });
            case "kubernetes:flowcontrol.apiserver.k8s.io/v1alpha1:FlowSchemaList":
                return new flowSchemaList_1.FlowSchemaList(name, undefined, { urn });
            case "kubernetes:flowcontrol.apiserver.k8s.io/v1alpha1:FlowSchemaPatch":
                return new flowSchemaPatch_1.FlowSchemaPatch(name, undefined, { urn });
            case "kubernetes:flowcontrol.apiserver.k8s.io/v1alpha1:PriorityLevelConfiguration":
                return new priorityLevelConfiguration_1.PriorityLevelConfiguration(name, undefined, { urn });
            case "kubernetes:flowcontrol.apiserver.k8s.io/v1alpha1:PriorityLevelConfigurationList":
                return new priorityLevelConfigurationList_1.PriorityLevelConfigurationList(name, undefined, { urn });
            case "kubernetes:flowcontrol.apiserver.k8s.io/v1alpha1:PriorityLevelConfigurationPatch":
                return new priorityLevelConfigurationPatch_1.PriorityLevelConfigurationPatch(name, undefined, { urn });
            default:
                throw new Error(`unknown resource type ${type}`);
        }
    },
};
pulumi.runtime.registerResourceModule("kubernetes", "flowcontrol.apiserver.k8s.io/v1alpha1", _module);
//# sourceMappingURL=index.js.map
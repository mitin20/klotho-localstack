...
describe('entrypoint.module', () => {
    describe('backportRun', () => {
        describe('when running into merge conflict', () => {
            let response;
            beforeAll(async () => {
                response = (await (0, entrypoint_module_1.backportRun)({
                    options: {
        ...
        describe('when target branch in branchLabelMapping is invalid', () => {
            ...
            it('should correct error code', async () => {
                ...
                expect(response.error.message).toBe('The branch "--foo" does not exist');
            });
        });
    ...
    describe('getCommits', () => {
        it('pullNumber', async () => {
            const commits = await (0, entrypoint_module_1.getCommits)({
      ...
      `);
        ...
        it('throws when missing a filter', async () => {
            await expect(() => (0, entrypoint_module_1.getCommits)({
                accessToken: accessToken,
                repoName: 'kibana',
                repoOwner: 'elastic',
                maxNumber: 3,
            })).rejects.toThrowError('Must supply one of: `pullNumber`, `sha`, `prFilter` or `author`');
        });
    });
});
